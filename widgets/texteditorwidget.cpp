...
void TextEditorWidget::keyPressEvent(QKeyEvent *e)
{
    ...
    // сейчас работает так, это все не баги, а фичи:
    // если открыл скобку ("(", "{" или "[") и сразу закрыл, то вторую не пишет, а перепрыгивает за нее
    // если просто открыл, то закрывающую ставит сам и пишешь внутри скобок
    // И! если открыл "(", то не поставишь внутрии нее "}", он ее воспримет как закрывающкю ")" (и наоборот)
    // то же самое для открывающей "(" или "{" и закрывающей "]", но не наоборот, т.к. Shift - отдельное нажатие
    // И! если отпустить Shift между нажатиями "(" и ")", то ")" не самоудалится, т.к. Shift - отдельное нажатие
